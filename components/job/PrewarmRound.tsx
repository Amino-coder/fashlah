...
// never touches job_answers / job_votes / job_round_results, so it
...
export default function PrewarmRound({
  ...
  const [votes, setVotes] = useState<JobPrewarmVoteRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error: err } = await supabase
        .from("job_prewarm_round_prompts")
        .select("round_number, job_prewarm_prompts(text_ar, text_en)")
      ...
      const rows: PromptRow[] = data.map((r: any) => ({
        ...
        text_ar: r.job_prewarm_prompts?.text_ar ?? "",
        text_en: r.job_prewarm_prompts?.text_en ?? "",
      }));
      setPrompts(rows);
      setLoading(false);
    })();
  ...
  async function refetchVotes() {
    const { data } = await supabase
      .from("job_prewarm_votes")
      .select("*")
      .eq("session_id", session.id);
    setVotes((data as JobPrewarmVoteRow[]) || []);
  }

  useEffect(() => {
    refetchVotes();
    const channel = supabase
      .channel(`job-prewarm-${session.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "job_prewarm_votes", filter: `session_id=eq.${session.id}` },
        () => refetchVotes())
      .subscribe();
    // Polling fallback, matching the rest of Job's realtime-with-backup pattern.
  ...
  async function pick(votedForPlayerId: string) {
    ...
    const { error: insertErr } = await supabase.from("job_prewarm_votes").insert({
  ...
  if (error && !prompts) {
    return (
      <div style={{ padding: "40px 24px", textAlign: "center" }}>
        <p style={{ color: "#3B82F6", fontWeight: 700 }}>{error}</p>
      </div>
    );
  }
  ...
  return (
    <div className="screen-enter" style={{ padding: "12px 24px 32px", display: "flex", flexDirection: "column", gap: 18, position: "relative" }}>
      ...
      <div className="progress-track">
        <div className="progress-fill" style={{ width: `${progressPct}%`, background: "linear-gradient(90deg, #3B82F6, #1E40AF)" }} />
      </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isSelected ? "linear-gradient(135deg, #3B82F6, #1E40AF)" : "var(--card)",
      ...
      {error && (
        <p className="font-body" style={{ fontSize: 12, color: "#3B82F6", textAlign: "center" }}>{error}</p>
      )}
    </div>
  );
}
...
function PrewarmTeaser({
  ...
  const { topPlayer, topCount, isTie } = useMemo(() => {
    let top: JobPlayerRow | null = null;
    let count = 0;
  ...
  async function advanceToRound1() {
    ...
    advancingRef.current = true;
    const { error: err } = await supabase.from("job_sessions")
      ...
      {topPlayer && !isTie && topCount >= 2 ? (
        <>
          ...
          <h2 className="font-display" style={{ fontSize: 22, fontWeight: 800, lineHeight: 1.5 }}>
            {lang === "ar"
              ? `يبدون الكل شايفين ${topPlayer.nickname} ما بيعدي المقابلة 💀`
              : `Looks like nobody thinks ${topPlayer.nickname} is getting past the interview 💀`}
          </h2>
        </>
      ) : (
        <h2 className="font-display" style={{ fontSize: 22, fontWeight: 800, lineHeight: 1.5 }}>
          {lang === "ar" ? "كلكم مرشحين بنفس القوة 👀" : "Everyone looks equally qualified, apparently 👀"}
        </h2>
      )}
