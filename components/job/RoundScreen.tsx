...
import type {
  JobSessionRow, JobPlayerRow, JobPromptRow,
  JobAnswerRow, JobVoteRow,
} from "@/lib/job-types";
...
export default function RoundScreen({
  ...
  const [prompt, setPrompt] = useState<JobPromptRow | null>(null);
  const [answers, setAnswers] = useState<JobAnswerRow[]>([]);
  const [votes, setVotes] = useState<JobVoteRow[]>([]);
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        ...
        .select("prompt_id, job_prompts(*)")
        .eq("session_id", session.id)
        .eq("round_number", session.current_round)
        .single();
      const p = (data as any)?.job_prompts as JobPromptRow | undefined;
      setPrompt(p ?? null);
    })();
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      const { data: ans } = await supabase
        .from("job_answers").select("*")
        .eq("session_id", session.id).eq("round_number", session.current_round);
      const { data: vt } = await supabase
        .from("job_votes").select("*")
        .eq("session_id", session.id).eq("round_number", session.current_round);
      if (cancelled) return;
      setAnswers((ans as JobAnswerRow[]) || []);
      setVotes((vt as JobVoteRow[]) || []);
    }
    loadAll();

    const channel = supabase
      .channel(`job-round-${session.id}-${session.current_round}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "job_answers", filter: `session_id=eq.${session.id}` }, loadAll)
      .on("postgres_changes", { event: "*", schema: "public", table: "job_votes", filter: `session_id=eq.${session.id}` }, loadAll)
      .subscribe();
  ...
  async function advancePastCountdown() {
    ...
    const { error } = await supabase.from("job_sessions")
  ...
  async function computeRoundResult() {
    try {
      const res = await fetch("/api/job-round-result", {
        method: "POST",
  ...
  async function submitAnswer() {
    ...
    await supabase.from("job_answers").insert({
  ...
  async function castVote(answerId: string, answerPlayerId: string) {
    ...
    await supabase.from("job_votes").insert({
      session_id: session.id, round_number: session.current_round,
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20, marginTop: 60 }}>
        ...
        <div
          ...
          style={{ fontSize: 80, fontWeight: 800, color: BLUE, lineHeight: 1 }}
        >
          {remaining > 0 ? remaining : (lang === "ar" ? "يلا!" : "Go!")}
        </div>
        {remaining <= 0 && (
          <div style={{ color: BLUE, height: 6 }}>
        ...
        {countdownError && (
          <p className="font-body" style={{ fontSize: 12, color: BLUE, textAlign: "center", maxWidth: 260 }}>
            {countdownError}
          </p>
        )}
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18, marginTop: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <div
          ...
          style={{
            ...
            background: remaining <= 5 ? BLUE : `linear-gradient(135deg, ${BLUE}, ${NAVY})`,
          }}
        >
          {remaining}
        </div>
      ...
      {session.round_phase === "answering" && (
        <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: draft.trim() ? `linear-gradient(135deg, ${BLUE}, ${NAVY})` : "var(--ring)",
          ...
          {advanceError && (
            <p className="font-body" style={{ fontSize: 12, color: BLUE, textAlign: "center" }}>{advanceError}</p>
          )}
        </>
      )}
      ...
      {(session.round_phase === "voting" || session.round_phase === "reveal") && (
        <>
          {session.round_phase === "reveal" ? (
            <p className="font-display pop" style={{ textAlign: "center", fontSize: 20, fontWeight: 800, color: BLUE, margin: 0 }}>
              🔒 {lang === "ar" ? "تم قفل التصويت!" : "Vote locked!"}
            </p>
          ) : (
            <p className="font-display" style={{ textAlign: "center", fontSize: 20, fontWeight: 800, color: BLUE, margin: 0 }}>
              {t.voteHeader}
            </p>
          )}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: (isSelected || isCommitted) ? `3px solid ${BLUE}` : "3px solid transparent",
                    opacity: 1,
                ...
                >
                  ...
                  {revealed && author && (
                    <span
                      ...
                      style={{
                        fontSize: 11, fontWeight: 700, color: BLUE, display: "flex", alignItems: "center", gap: 4,
          ...
          {session.round_phase === "voting" && !myVote && (
            <button
              ...
              style={{
                ...
                background: selectedAnswerId ? `linear-gradient(135deg, ${BLUE}, ${NAVY})` : "var(--ring)",
          ...
          {scoringError && (
            <p className="font-body" style={{ fontSize: 12, color: BLUE, textAlign: "center" }}>{scoringError}</p>
          )}
        </>
      )}
    </div>
