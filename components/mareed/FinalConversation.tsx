...
export default function FinalConversation({
  ...
  // API call to finish writing to mareed_round_results). That's a real
  ...
  useEffect(() => {
    ...
    async function attempt(tryNumber: number) {
      const { data: prompts } = await supabase
        ...
        .select("round_number, mareed_prompts(text_ar, text_en)")
        .eq("session_id", session.id)
        .order("round_number");

      const { data: results } = await supabase
        .from("mareed_round_results")
        .select("round_number, mareed_answers(text), mareed_players(nickname, avatar_emoji)")
        .eq("session_id", session.id)
        .order("round_number");
      ...
      for (const p of prompts || []) {
        ...
        const promptRow = (p as any).mareed_prompts;
        const answerRow = r.mareed_answers;
        const playerRow = r.mareed_players;
