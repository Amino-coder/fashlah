...
import type { MareedSessionRow, MareedPlayerRow, MareedPrewarmVoteRow } from "@/lib/mareed-types";
...
// never touches mareed_answers / mareed_votes / mareed_round_results, so it
...
export default function PrewarmRound({
  ...
  const [votes, setVotes] = useState<MareedPrewarmVoteRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error: err } = await supabase
        .from("mareed_prewarm_round_prompts")
        .select("round_number, mareed_prewarm_prompts(text_ar, text_en)")
      ...
      const rows: PromptRow[] = data.map((r: any) => ({
        ...
        text_ar: r.mareed_prewarm_prompts?.text_ar ?? "",
        text_en: r.mareed_prewarm_prompts?.text_en ?? "",
      }));
      setPrompts(rows);
      setLoading(false);
    })();
  ...
  async function refetchVotes() {
    const { data } = await supabase
      .from("mareed_prewarm_votes")
      .select("*")
      .eq("session_id", session.id);
    setVotes((data as MareedPrewarmVoteRow[]) || []);
  }

  useEffect(() => {
    refetchVotes();
    const channel = supabase
      .channel(`mareed-prewarm-${session.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "mareed_prewarm_votes", filter: `session_id=eq.${session.id}` },
        () => refetchVotes())
      .subscribe();
    // Polling fallback, matching the rest of Mareed's realtime-with-backup pattern.
  ...
  async function pick(votedForPlayerId: string) {
    ...
    const { error: insertErr } = await supabase.from("mareed_prewarm_votes").insert({
...
function PrewarmTeaser({
  ...
  const { topPlayer, topCount, isTie } = useMemo(() => {
    let top: MareedPlayerRow | null = null;
    let count = 0;
  ...
  async function advanceToRound1() {
    ...
    advancingRef.current = true;
    const { error: err } = await supabase.from("mareed_sessions")
