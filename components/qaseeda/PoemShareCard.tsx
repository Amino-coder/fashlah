...
/**
 * A beautifully designed poetry card, not a game results screen — so unlike
 * the in-app reveal (which credits each line's author as it lands), the
 * shareable card keeps the poem itself as the only hero: just the lines,
 * then one collective "كتبها" credit for the whole group at the bottom.
 *
 * Laid out with real DOM flow (flexbox) at the card's native 1080x1920 and
 * scaled down via CSS transform, the same technique عبارات's QuoteCard
 * uses — except there's no fixed background image here, so a poem of any
 * length reflows naturally instead of needing a hand-tuned font ladder.
 */
export default function PoemShareCard({
  poem, code, cheerLabel, titleLabel, creditLabel, playerNames, scale,
}: {
  poem: PoemLine[];
  code: string;
  cheerLabel: string;
  ...
  const poemFontSize = poemFontSizeFor(lineUnitsFor(poem));
