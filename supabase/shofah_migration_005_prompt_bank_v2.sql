-- ============================================================================
-- VERSION MARKER: FIX-CONFIRMED-V3 — if you don't see this exact line when
-- you open the file, you're looking at a stale/cached copy, not this one.
-- ============================================================================
-- SHOFAH migration 005 — prompt bank v2 (27 prompts, audience-aware)
-- Adds an `audience` column: null = shown to everyone, 'girl'/'guy' = only
-- drawn when that character was picked. Replaces the entire old 48-prompt
-- bank with the new, sharper 27-prompt set.
--
-- IMPORTANT: this deletes existing shofah_round_prompts rows first, since
-- they reference shofah_prompts.id and would block the delete otherwise.
-- If you have an in-progress test session, run shofah_reset_test_data.sql
-- first (or just accept losing test data — recommended before running this).
-- ============================================================================

alter table shofah_prompts
  add column audience text check (audience in ('girl', 'guy'));
