...
-- visits this month — each shown as 'completed' or 'in_progress/left'
-- based on whether a matching 'complete' event exists for that visit.
...
from bidal_sessions s where s.created_at >= (date_trunc('month', now() AT TIME ZONE 'Asia/Riyadh') AT TIME ZONE 'Asia/Riyadh')
union all
