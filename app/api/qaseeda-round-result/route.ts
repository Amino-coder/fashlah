...
// (qaseeda_compute_round_result — see qaseeda_schema.sql), so this route
// makes exactly ONE database round-trip instead of several. Idempotency
// (safe to call twice for the same round) is handled inside that function.
// The recorded winner is what becomes the next official line of the poem.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc("qaseeda_compute_round_result", {
