...
import { MAREED_ANSWER_MAP, MAREED_CATEGORY_FALLBACK, pickTwoDistinct } from "@/lib/demo/demoContent";
 ...
 * components/mareed/ or app/mareed/session/, so there is zero risk of
 * this feature touching the real multiplayer flow.
 *
 * The one read-only exception: on mount, this fetches 5 real prompts from
 * mareed_prompts (public SELECT, same table the real game draws from) so
 * the demo genuinely plays out the same questions as an actual game.
 * Bot answers are looked up by exact prompt text (MAREED_ANSWER_MAP,
 ...
 */
export default function MareedDemoPage() {
  useEffect(() => { trackPageView("mareed_demo"); }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          const fallback = Object.keys(MAREED_ANSWER_MAP).map((text_ar) => ({ text_ar, category: "wildcard" }));
  ...
  return prompts ? <MareedDemoGame prompts={prompts} /> : null;
}

function MareedDemoGame({ prompts }: { prompts: PromptRow[] }) {
  function getBotAnswers(round: number): [string, string] {
    const p = prompts[(round - 1) % prompts.length];
    return MAREED_ANSWER_MAP[p.text_ar] ?? MAREED_CATEGORY_FALLBACK[p.category] ?? pickTwoDistinct(Object.values(MAREED_ANSWER_MAP).flat());
  }
      ...
      <div dir="rtl" style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--ink)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "24px", position: "relative", zIndex: 1 }}>
          <DemoDrumrollResults
            ...
            winnerIsText="الشخص اللي عنده مرض نفسي هو..."
            connectorEmoji="🧠"
            congratsText="🧠 رسمي!"
            othersText="أما الباقين... لسه بخير 😂"
            createHref="/mareed/create"
          />
        </div>
      </div>
    );
