...
export default function MareedWaitingRoom() {
  const { code } = useParams<{ code: string }>();
  const { lang, dark, ready } = usePrefs();
  const t = MAREED_STR[lang as MareedLang];

  const [session, setSession] = useState<MareedSessionRow | null>(null);
  const [players, setPlayers] = useState<MareedPlayerRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: sess, error: sessErr } = await supabase
        .from("mareed_sessions").select("*").eq("code", code.toString().toUpperCase()).single();
      if (sessErr || !sess) { setError(t.errorNotFound); return; }
      setSession(sess as MareedSessionRow);

      const { data: pl } = await supabase
        .from("mareed_players").select("*").eq("session_id", sess.id).order("joined_at", { ascending: true });
      setPlayers((pl as MareedPlayerRow[]) || []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`mareed-session-${session.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "mareed_players", filter: `session_id=eq.${session.id}` },
        async () => {
          const { data: pl } = await supabase
            .from("mareed_players").select("*").eq("session_id", session.id).order("joined_at", { ascending: true });
          setPlayers((pl as MareedPlayerRow[]) || []);
        })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "mareed_sessions", filter: `id=eq.${session.id}` },
        (payload) => setSession(payload.new as MareedSessionRow))
      .subscribe();
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      const { data: pl } = await supabase
        .from("mareed_players").select("*").eq("session_id", session.id).order("joined_at", { ascending: true });
      if (pl) setPlayers(pl as MareedPlayerRow[]);
      const { data: sess } = await supabase.from("mareed_sessions").select("*").eq("id", session.id).single();
      if (sess) setSession(sess as MareedSessionRow);
    }, 1200);
  ...
  async function startGame() {
    ...
    try {
      const { data: existingRounds } = await supabase
        .from("mareed_round_prompts").select("id").eq("session_id", session.id).limit(1);
      ...
      if (!existingRounds || existingRounds.length === 0) {
        ...
        const { error: rpErr } = await supabase.from("mareed_round_prompts").insert(rows);
      ...
      if (!existingPrewarmRounds || existingPrewarmRounds.length === 0) {
        const { data: allPrewarmPrompts, error: prewarmPromptsErr } = await supabase
          .from("mareed_prewarm_prompts").select("id")
          .eq("active", true);
        ...
        const { error: prwErr } = await supabase.from("mareed_prewarm_round_prompts").insert(prewarmRows);
      ...
      if (updated) setSession(updated as MareedSessionRow);
    } catch (e: any) {
  ...
  return (
    <div dir={t.dir} className={dark ? "dark" : ""} style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--ink)", position: "relative", overflow: "hidden" }}>
      <Blobs />
      {(error || !session || session.status === "waiting") && <HomeButton label={t.backHome} href="/mareed" />}
      ...
      {session && session.status === "waiting" && (
        <div style={{ position: "absolute", top: 24, insetInlineEnd: 24, zIndex: 2 }}>
          <HelpButton game="mareed" lang={lang} autoOpenFirstVisit={false} />
        </div>
      )}
