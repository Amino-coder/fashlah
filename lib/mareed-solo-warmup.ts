/**
 * مريض نفسي solo mode's warm-up questions — inherited as-is from شوفة's
 * own solo warmup (itself forked from وش شخصيتك's ROUND1_POOL in
 * lib/wadak-content.ts). The question content is generic personality
 * trivia (mood, mornings, exams...) with nothing dating/marriage-specific
 * in the text itself, so it was left unchanged when شوفة became مريض
 * نفسي — only the RESULT framing changed (see app/mareed/solo/page.tsx:
 * "married" became "diagnosed" throughout). Kept as separate content from
 * both of those other files so editing this one never silently changes
 * شوفة's or وش شخصيتك's actual games.
 */
...
export const MAREED_WARMUP_QUESTIONS: WarmupQuestion[] = [
  {
...
export const MAX_LUCKY = MAREED_WARMUP_QUESTIONS.length; // 6
...
export const DIAGNOSED_THRESHOLD = 3;
